...
export default function AllBrandcomponent() {
  ...
  const getBrandProducts = async () => {
    try {
      const { data } = await axios.get(
        `${baseURL}/brand/${brandSlug}/${brandId}/product/slugandid`
      );
      ...
      if (data.data.length) {
        setBrandInfo(data.data[0].brandId);
      }
    } catch (err) {
      console.log(err);
    } finally {
  ...
  useEffect(() => {
    ...
    filtered = filtered.filter((product) => product.finalPrice <= priceRange);
  ...
  }, [priceRange, inStockOnly, sortBy, brandProducts]);

  return (
    <>
      {/* ================= BANNER ================= */}
      <div className="relative h-[260px]">
        {brandInfo?.image?.secure_url ? (
          <img
            src={brandInfo.image.secure_url}
            alt={brandInfo.name}
        ...
        <div className="absolute inset-0 bg-black/50 flex items-center px-8 text-white">
          <div>
