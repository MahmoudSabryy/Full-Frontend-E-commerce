...
export default function AllSubCategoryComponent() {
    ...
    const getProducts = async () => {
      try {
        const { data } = await axios.get(
          `${baseURL}/subcategory/slug/${subcategorySlug}/product/slug/related/sub`
        );
        ...
        if (data.data.length) {
          setSubcategoryInfo(data.data[0].subcategoryId);
        }
      } catch (err) {
        console.log(err);
      } finally {
  ...
  return (
    <>
      ...
      <div className="relative h-[260px]">
        {subcategoryInfo?.image?.secure_url ? (
          <img
            src={subcategoryInfo.image.secure_url}
        ...
        <div className="absolute inset-0 bg-black/50 flex items-center px-8 text-white">
          <div>
            <h1 className="text-4xl font-extrabold capitalize">
              {subcategoryInfo?.name || subcategorySlug}
            </h1>
